...
class AutoregressiveDMLLShape(Hidden2OutputShape):
    ...
    def sample_shape(self, x, class_labels, translations, angles, sizes): # This is the version 3
        ...
        for i in range(self.shape_embed):
            if coord1 is None:
                x = self.transformer.drop(points + position_embeddings[:, :1, :])
                for block in self.transformer.blocks[:6]:
                    x = block(x)
                point_x = self.shape_layer_x(self.ln_x(x))
                coord1 = sample(point_x)
                # import ipdb
                # ipdb.set_trace()
                x_token_embeddings = self.pos_x(coord1)
                x = x + x_token_embeddings + position_embeddings[:, :1, :]
                for block in self.transformer.blocks[6:8]:
                    x = block(x)
                point_y = self.shape_layer_y(self.ln_x(x))
                coord2 = sample(point_y)
                y_token_embeddings = self.pos_y(coord2)
                x = x + x_token_embeddings + y_token_embeddings + position_embeddings[:, :1, :]
                for block in self.transformer.blocks[8:10]:
                    x = block(x)
                point_z = self.shape_layer_z(self.ln_z(x))
                coord3 = sample(point_z)
                z_token_embeddings = self.pos_z(coord3)
                ...
                for block in self.transformer.blocks[10:12]:
                    x = block(x)
                latents = self.shape_layer_latent(self.ln_latent(x))
                ...
                y_token_embeddings = self.pos_y(coord2)
                z_token_embeddings = self.pos_z(coord3)
                latent_token_embeddings = self.pe_latent(latent)
                token_embeddings = torch.cat([points, latent_token_embeddings + x_token_embeddings + y_token_embeddings + z_token_embeddings], dim=1)
                embeddings = token_embeddings + position_embeddings[:, :token_embeddings.shape[1], :]
                ...
                coord1 = torch.cat((coord1, idx), dim=1)
                x_token_embeddings = self.pos_x(coord1)
                x = x + x_token_embeddings + position_embeddings[:, :token_embeddings.shape[1], :]
                for block in self.transformer.blocks[6:8]:
                    x = block(x)
                point_y = self.shape_layer_y(self.ln_x(x))
                idx = sample(point_y)
                coord2 = torch.cat((coord2, idx), dim=1)
                y_token_embeddings = self.pos_y(coord2)
                x = x + x_token_embeddings + y_token_embeddings + position_embeddings[:, :token_embeddings.shape[1], :]
                for block in self.transformer.blocks[8:10]:
                    x = block(x)
                point_z = self.shape_layer_z(self.ln_z(x))
                idx = sample(point_z)
                coord3 = torch.cat((coord3, idx), dim=1)
                z_token_embeddings = self.pos_z(coord3)
                x = x + x_token_embeddings + y_token_embeddings + z_token_embeddings + position_embeddings[:, :token_embeddings.shape[1], :]
                for block in self.transformer.blocks[10:12]:
                    x = block(x)
                latents = self.shape_layer_latent(self.ln_latent(x))
                idx = sample(latents)
                latent = torch.cat((latent, idx), dim=1)
        ...
        #             x = block(x)
        #         point_x = self.shape_layer_x(self.ln_x(x))
        #         coord1 = sample_from_dmll(point_x.reshape(-1, 30))
        #         x_token_embeddings = self.pe_x(coord1)
        #         x = x + x_token_embeddings + position_embeddings[:, :1, :]
        #         for block in self.transformer.blocks[6:8]:
        #             x = block(x)
        #         point_y = self.shape_layer_y(self.ln_x(x))
        #         coord2 = sample_from_dmll(point_y.reshape(-1, 30))
        #         y_token_embeddings = self.pe_y(coord2)
        #         x = x + x_token_embeddings + y_token_embeddings + position_embeddings[:, :1, :]
        #         for block in self.transformer.blocks[8:10]:
        #             x = block(x)
        #         point_z = self.shape_layer_z(self.ln_z(x))
        #         coord3 = sample_from_dmll(point_z.reshape(-1, 30))
        #         z_token_embeddings = self.pe_z(coord3)
        #         x = x + x_token_embeddings + y_token_embeddings + z_token_embeddings + position_embeddings[:, :1, :]
        #         for block in self.transformer.blocks[10:12]:
        #             x = block(x)
        #         latents = self.shape_layer_latent(self.ln_latent(x))
        ...
        #             x = block(x)
        #         point_x = self.shape_layer_x(self.ln_x(x))
        #         idx = sample_from_dmll(point_x[:, -1].reshape(-1, 30))
        #
        #         coord1 = torch.cat((coord1, idx), dim=1)
        #         x_token_embeddings = self.pe_x(coord1)
        #         x = x + x_token_embeddings + position_embeddings[:, :token_embeddings.shape[1], :]
        ...
        #             x = block(x)
        #         point_y = self.shape_layer_y(self.ln_x(x))
        #         idx = sample_from_dmll(point_y[:, -1].reshape(-1, 30))
        #         coord2 = torch.cat((coord2, idx), dim=1)
        #         y_token_embeddings = self.pe_y(coord2)
        #         x = x + x_token_embeddings + y_token_embeddings + position_embeddings[:, :token_embeddings.shape[1], :]
        #         for block in self.transformer.blocks[8:10]:
        #             x = block(x)
        #         point_z = self.shape_layer_z(self.ln_z(x))
        #         idx = sample_from_dmll(point_z[:, -1].reshape(-1, 30))
        #         coord3 = torch.cat((coord3, idx), dim=1)
        #         z_token_embeddings = self.pe_z(coord3)
        #         x = x + x_token_embeddings + y_token_embeddings + z_token_embeddings + position_embeddings[:, :token_embeddings.shape[1], :]
        #         for block in self.transformer.blocks[10:12]:
        ...
        # point_x = sample_from_dmll(shape_x.reshape(B * L * 512, -1)).reshape(B, L, 512, -1)
        ...
        # point_y = sample_from_dmll(shape_y.reshape(B*L*512, -1)).reshape(B, L, 512, -1)
        ...
        # point_z = sample_from_dmll(shape_z.reshape(B * L * 512, -1)).reshape(B, L, 512, -1)
    ...
    def forward(self, x, sample_params):
        ...
        points = self.shape_encoder(points) #B, L, C
        x_embd = self.pe_x(shapes[..., 0])
        y_embd = self.pe_y(shapes[..., 1])
        z_embd = self.pe_z(shapes[..., 2])
        latent_embd = self.pe_latent(shapes[..., 3].long())
        # points = points.unsqueeze(2).repeat(1, 1, 512, 1)
        points = points.unsqueeze(2) #B, L, 1, C.
        #This is one way, you can also refer to another one with k-1 to predict k.
        # self.pe_x(), revist with different strategy.
        # ipdb.set_trace()
        cond = torch.cat((points, x_embd + y_embd + z_embd + latent_embd), dim=2)
        points = cond[:, 0, :-1] + self.pos_emb
        # ipdb.set_trace()
        # points = points + self.pos_emb #latent_embd
        # points = points.squeeze(1)
        for block in self.transformer.blocks[:6]:
        ...
        for block in self.transformer.blocks[6:8]:
            points = block(points)
        shape_y = self.shape_layer_y(self.ln_y(points))
        points = points.unsqueeze(1) + y_embd + x_embd
        points = points.squeeze(1)
        for block in self.transformer.blocks[8:10]:
            points = block(points)
        shape_z = self.shape_layer_z(self.ln_z(points))
        points = points.unsqueeze(1) + z_embd + y_embd + x_embd
        points = points.squeeze(1)
        for block in self.transformer.blocks[10:12]:
            points = block(points)
        ...
        # ipdb.set_trace()
        shapes = (
            F.log_softmax(shape_x, dim=-1),
            F.log_softmax(shape_y, dim=-1),
            F.log_softmax(shape_z, dim=-1),
