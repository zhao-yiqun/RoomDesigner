...
class ClassEncoder_12(nn.Module):
    """Container module with an encoder, a recurrent or transformer module, and a decoder."""

    def __init__(self, ninp, nhead, nlayers, nclasses=55, coord_vocab_size=256, latent_vocab_size=512, reso=128):
        #nip:1024. nhead:16, reso:512
        super(ClassEncoder_12, self).__init__()
        self.reso = reso
    ...
    def sample(self, cond):
        ...
        for i in range(self.reso):
            if coord1 is None:
                ...
                x = x + x_token_embeddings + y_token_embeddings + z_token_embeddings + position_embeddings[:, :1, :]
                for block in self.transformer.blocks[10:]:
                    x = block(x) # B x S x C
                latent_logits = self.latent_head(self.ln_latent(x))
                ix = sample(latent_logits)
                latent = ix
...
#ClassEncoder_12
@register_model
def class_encoder_21_512_1024_24_K1024(pretrained=False, **kwargs):
    # model = ClassEncoder(
    #     ninp=1024,
    #     nhead=16,
    #     nlayers=24,
...
#ClassEncoder_12
@register_model
def class_encoder_24_512_1024_24_K1024(pretrained=False, **kwargs):
    model = ClassEncoder_12(
        ninp=1024,
        nhead=16,
        nlayers=12,
