...
# where can I get the shape code?
class ShapeScene(object):
    """Contains all the information for a scene."""
    """include a shape code and its coords in a scene"""
    def __init__(self, scene_id, scene_type, bboxes, shapecode):
        self.bboxes = bboxes
        self.scene_id = scene_id
        self.scene_type = scene_type
    ...
    def ordered_shape_with_centorid(self):
        centroids = np.array([b.centroid for b in self.bboxes])
        ordering = np.lexsort(centroids.T)
        shapecodes = [self.shapecode[i] for i in ordering]

        return shapecodes

    def ordered_shape_with_centroid(self, all_labels):
        centroids = np.array([b.centroid for b in self.bboxes])
        int_labels = np.array([[b.int_label(all_labels)] for b in self.bboxes])
        ordering = np.lexsort(np.hstack([centroids, int_labels]).T)
        shapecodes = [self.shapecode[i] for i in ordering]
