...
def get_raw_dataset(
    ...
    if "shape" in dataset_type:
        ...
        dataset = CachedThreedFrontShape(
            config["dataset_directory"],
            config=config,
            scene_ids=split_scene_ids,
            splits=split
        )
    elif "open" in dataset_type:
        splits_builder = CSVSplitsBuilder(config["annotation_file"])
        split_scene_ids = splits_builder.get_splits(split)

        dataset = CachedThreedFrontOpen(
            config["dataset_directory"],
            config=config,
            scene_ids=split_scene_ids,
            splits=split
...
def get_dataset_raw_and_encoded(
    ...
    encoding_config = config.get("encoding_type")


    if "shape" in encoding_config:
        print(encoding_config, config.get("stage") )
        # ipdb.set_trace()
        if config.get("stage") == "first":
        ...
        encoding = dataset_encoding_factory_shape(
            encoding_config,
            dataset,
            augmentations,
            config.get("box_ordering", None)
        )
    elif "open" in encoding_config:
        print(encoding_config, config.get("stage"))
        encoding = dataset_encoding_factory_open(
            encoding_config,
            dataset,
            augmentations,
            config.get("box_ordering", None)
        )
    elif "sceneformer" in encoding_config:
        encoding = dataset_encoding_factory_sceneformer(
            encoding_config,
            dataset,
            augmentations,
            config.get("box_ordering", None)
