...
class ThreedFutureDataset(object):
    ...
    def get_closest_furniture_to_box(self, query_label, query_size):
        ...
        #FixMe: desige for how many object to further retrieve.
        return sorted_mses[0]

    def get_closest_5_furniture_to_box(self, query_label, query_size):
        objects = self._filter_objects_by_label(query_label)
        mses = {}
        for i, oi in enumerate(objects):
            mses[oi] = np.sum((oi.size - query_size)**2, axis=-1)
        ...
        # ind = np.random.randint(0, 1)
        #FixMe: desige for how many object to further retrieve.
        return sorted_mses[4]

    def get_closest_furniture_to_shape(self, query_label, query_shape, querey_size):
        objects = self._filter_objects_by_label(query_label)

        mses = {}
        for i, oi in enumerate(objects):
            mses[oi] = np.sum((oi.shape - query_shape) ** 2, axis=-1)
        sorted_mses = [k for k, v in sorted(mses.items(), key=lambda x: x[1])]

        ind = np.random.randint(0,5)
        return sorted_mses[ind]
