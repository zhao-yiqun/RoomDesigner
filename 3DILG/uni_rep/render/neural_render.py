...
class NeuralRender(Renderer):
    ...
    def render_mesh(
        ...
        # canonical space and camara space

        # import ipdb
        # ipdb.set_trace()

        with dr.DepthPeeler(self.ctx, v_pos_clip.float() , mesh_t_pos_idx_fx3, [resolution * spp, resolution * spp]) as peeler:
        ...
        antialias_mask = dr.antialias(
            hard_mask.clone().contiguous(), rast, v_pos_clip.float() ,
            mesh_t_pos_idx_fx3)
