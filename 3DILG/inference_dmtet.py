...
parser.add_argument('--model', default='vqvae_512_1024_2048_dmtet_90', type=str,
                    metavar='MODEL', help='Name of model to train')
parser.add_argument('--pth', default='output/vqvae_512_1024_2048_dmtet_90_fov39/checkpoint-519.pth', type=str)
...
from Front_dmtet import Dmtet_3DFRONT, FRONT_category
...
def main():
    ...
    with torch.no_grad():
        ...
        for category, _ in FRONT_category.items():
            ...
            dataset_test = Dmtet_3DFRONT(args.data_path, split='train', transform=None, sampling=False, return_surface=True,
                                    surface_sampling=True, pc_size=2048)
            dataset_test.file_paths = dataset_test.file_paths[:100]
            dataset_test.models = dataset_test.models[:100]
            ...
            for batch in metric_logger.log_every(data_loader_test, 10, header):
                ...
                surface, images, depths, condinfo, xyz = batch
                # import ipdb
                # ipdb.set_trace()
                ...
                surface2048 = surface.to(device, non_blocking=True)
                ...
                sdf_reg_loss_entropy, v_list, f_list, z_e_x, z_q_x, sigma, loss_commit, perplexity, depth_out, antilias_mask, all_detach = model(surface2048, condinfo)

                # import ipdb
                # ipdb.set_trace()
                for i in range(len(v_list)):
                    m = trimesh.Trimesh(v_list[i].cpu().numpy(), f_list[i].cpu().numpy())

                # import ipdb
                # ipdb.set_trace()

                # output = torch.cat([model.decoder(latents, centers, points[:, i * N:(i + 1) * N])[0] for i in
                ...
                m.export(f'vqvae_dmtet_90_epoch519/{xyz[0]}.obj')

            print(category, metric_logger.cd.avg, metric_logger.fscore.avg)
